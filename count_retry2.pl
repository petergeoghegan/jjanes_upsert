...
eval {
  ...
  my $sth=$dbh->prepare('update foo set count=count+? where index=? returning count');
  my $del=$dbh->prepare('delete from foo where index=? and count=0');
  my $ins=$dbh->prepare('insert into foo (index, count) values ($2,$1)');
  foreach (1..($ARGV[1]//1e6)) {
    ...
    RETRY: { 
      ...
      if (@$count == 0) {
         eval {$ins->execute($d,$i)}; ## duplicate key errors are expected upon races
         if (length $@) {
           redo RETRY if ('23505' eq $dbh->state());
           $@ =~ s/\n/\\n /g;
           die "insert failed for reason other than duplicate key, SQL state ", $dbh->state(), " $@";
         };
      } else {;
        #warn "key $i passed through $count->[0][0], eligible for deletion" if $count->[0][0]==0;
