...
create or replace function merge_db_exclusion(arg_index integer, arg_count integer) returns integer as
$$
declare
    fincount integer;
begin
    loop
        ...
        begin
            insert into upsert_race_test(index, count) values ($1, $2) returning count into fincount;
            return fincount;
        exception when exclusion_violation then
          -- now try to update the key
          update upsert_race_test set count = count + $2 where index = $1 returning count into fincount;
          if fincount is not null then
              return fincount;
          end if;
        end;
    end loop;
...
eval {
  ...
  my $sth=$dbh->prepare('select merge_db_exclusion($2,$1)');
